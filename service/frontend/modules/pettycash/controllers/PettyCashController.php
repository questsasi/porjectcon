...
use app\modules\pettycash\models\PettyCash;
...
class PettyCashController extends \yii\web\Controller
{
...
public function behaviors() {  
    return [
        ...
        'index'=>['get'],
        'get-petty-cash'=>['get'],
        'create-petty-cash'=>['post'],
        'update-petty-cash'=>['post'],
        'delete-petty-cash' => ['post'],         
        ],        
      ]
  ...
  public function actionIndex() {         
    $query= new Query;      
    $query ->from('petty_cash')      
    ->select("`id`, `project_id`, `account_id`, `amt`, `date_time`, `purpose`, `given_by`, `approved_by`");           
  ...
  public function actionUpdatePettyCash($id) {
    $params = Yii::$app->getRequest()->getBodyParams();  
    $model = $this->findModel($id);  
    $model->attributes=$params;
    if($model->save()) {      
  ...
  public function actionDeletePettyCash($id) {
    $model=$this->findModel($id);      
    if($model->delete()) { 
      $this->setHeader(200);
