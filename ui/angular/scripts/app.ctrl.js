 ...
 */
angular.module('app','materialModule')  
...
app.factory('httpApi', ['$q', '$http', function($q, $http){
    var _config = {};
    _config.cache = 'false';
    _config.async = 'false';

    var obj = {};
    obj.get = function(_url) {
    ...
    obj.post = function (_url, _params,_headers) {
        var deffered = $q.defer();
        var resolved = false;
      
        $http.post(_url, _params, _config).then(function (response) {
            deffered.resolve(response);
            resolved = true;
        }, function (_err) {
            deffered.reject(_err);
